...
$ua->max_redirects(3);
$ua->transactor->name('"Mozilla/5.0 (Macintosh; Intel Mac OS X 10_7_3) AppleWebKit/534.54.16 (KHTML, like Gecko) Version/5.1.4 Safari/534.54.16"');
...
sub get_url_list {
	...
	for(my $i = 0; $i < $page_count; $i ++) {
		...
		$dom->find('a[class="bulletinLink "]')->each (
		sub{			
			if ($check->( $_->text )) {
		...
		eval {
			...
			given($data->{'type_code'}) {
				...
				when ('house') {
					...
					my $do = $dom->find('span[data-field="street-buildingId"]');
					if($do->size > 0) {
						$addr = $do->first->all_text;
					} else {
						$do = $dom->find('span[data-field="street"]');
						$addr = $do->first->all_text if $do->size > 0;
					}
				}
				when ('land') {
					# земельный участок
			...
			if ($id = Rplus::Util::Realty->find_similar(%$data, state_code => ['raw', 'work', 'suspended'])) {
				...
				eval {
					...
					# вытащим фото
					$dom->find('div[class="zoomLink"]')->each (
					sub {
